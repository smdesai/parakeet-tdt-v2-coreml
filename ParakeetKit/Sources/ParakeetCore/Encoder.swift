...
struct Encoder {
    ...
    /// [channels, len]) to `onSlice`, in window order. Single source of truth for
    /// the per-window encode math; `encode` collects + concatenates, while the
    /// pipelined path decodes each slice as it arrives. Slices are produced
